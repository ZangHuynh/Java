...
public class KiemThuPhongHoc {
	...
	static String tieuDe() {
		return String.format("%-10s%-15s%-15s%-15s%-20s", "Dãy nhà", "Mã phòng", "SLB�?", "Diện tích", "Ghi Chú");
	}
	...
	static void suaChung(int chon, PhongHoc phs) {
		...
		case 1:
			System.out.println("Nhập dãy nhà:");
			phs.setDayNha(scn.nextLine());
			break;
		case 2:
			System.out.println("Nhập diện tích: ");
			phs.setDienTich(scn.nextDouble());
			break;
		case 3:
			System.out.println("Nhập số bóng đèn: ");
			phs.setSoBongDen(scn.nextInt());
			break;
		}
		...
		if (ps == null) {
			...
			if (ps instanceof PhongLyThuyet) { // nếu là phòng lý thuyết
				...
				do {
					...
					if (chon == 4) {
						System.out.println("Nhập số lượng máy tính: ");
						phs.setSoLuongMayTinh(scn.nextInt());
					}
					if (chon == 5) {
						if (cnt > 1)
							DSPhong.sua(phs);
	...
	static void xuatPhongDatChuan() {
		System.out.println("\t\t\tDANH S�?CH PHÒNG ĐẠT CHUẨN\n\n");
		System.out.println(tieuDe());
		System.out.println("===============================================================================");
	...
	static void sortTheoTenDayNha() {
		...
		System.out.println("Dã sắp xếp thành công");
	}
	...
	static void xuatPhongMayTinh60() {
		if (DSPhong.DSPhongCoSoMayCo60().size() == 0)
			...
			System.out.println("\t\t\tDANH S�?CH PHÒNG CÓ 60 MÁY\n\n");
			System.out.println(tieuDe());
			System.out.println("===============================================================================");
	...
	static void xuatTongSoPhongHoc() {
		System.out.println("Có tổng cộng: " + DSPhong.getSoLuongPhong() + " phòng h�?c");
	}
